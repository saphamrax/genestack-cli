...
// gatewayRouteName maps a service key (as used in overrides.endpoints.hosts) to
// the Envoy HTTPRoute that genestack's setup-envoy-gateway.sh creates in the
// openstack namespace. Only services with a known route can be patched.
//
// NOTE: confirm these names against the target cluster with
// `kubectl -n openstack get httproute` (or genestack's gateway-api templates).
var gatewayRouteName = map[string]string{
	"keystone":       "keystone",
	"glance":         "glance",
	"nova":           "nova",
	"neutron":        "neutron",
...
// GatewayHostServices returns the service keys that can be targeted by a custom
// gateway hostname override, sorted. Used for validation.
func GatewayHostServices() []string {
	keys := make([]string, 0, len(gatewayRouteName))
	for k := range gatewayRouteName {
		keys = append(keys, k)
	}
	sort.Strings(keys)
	return keys
...
// gatewayRoutes emits a script that patches the genestack-created HTTPRoutes so
// each overridden service answers on its custom hostname. genestack owns the
// routes (backendRefs/parentRefs); we only override spec.hostnames. With no
// overrides the script is a header-only no-op.
func gatewayRoutes(c *model.Cluster) File {
	var b strings.Builder
	w := func(f string, a ...any) { fmt.Fprintf(&b, f, a...) }
	w("#!/usr/bin/env bash\n")
	w("# Generated by genestack-cli — overrides per-service gateway hostnames.\n")
	w("# Re-run `overrides upload` after editing overrides.endpoints.hosts.\n")
	w("set -uo pipefail\n\n")

	for _, svc := range sortedKeys(c.Overrides.Endpoints.Hosts) {
		fqdn := c.Overrides.Endpoints.Hosts[svc]
		route, ok := gatewayRouteName[svc]
		if !ok || fqdn == "" {
			continue
		}
		w("kubectl -n openstack patch httproute %s --type=merge \\\n", route)
		w("  -p '{\"spec\":{\"hostnames\":[\"%s\"]}}' || echo \"WARN: httproute %s not found\"\n", fqdn, route)
	}
